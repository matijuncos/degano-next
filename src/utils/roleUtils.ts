...
/**
 * Ofusca números de teléfono si el usuario no tiene permiso
 * @param phone - Número de teléfono a ofuscar
 * @param role - Rol del usuario
 * @param phoneType - Tipo de teléfono ('client' | 'show')
 */
export function obfuscatePhone(
  phone: string | undefined | null,
  role: UserRole | undefined,
  phoneType: 'client' | 'show' = 'client'
): string {
  if (!phone) return '';

  const permissions = getPermissions(role);

  // Verificar permiso según tipo de teléfono
  const canView =
    phoneType === 'client'
      ? permissions.canViewClientPhones
      : permissions.canViewShowPhones;

  if (canView) {
    return phone;
  }

  // Ofuscar completamente
  return '****';
}

/**
 * Ofusca múltiples teléfonos en un array
 */
export function obfuscatePhones<T extends { phoneNumber?: string }>(
  items: T[],
  role: UserRole | undefined,
  phoneType: 'client' | 'show' = 'client'
): T[] {
  return items.map((item) => ({
    ...
    phoneNumber: item.phoneNumber
      ? obfuscatePhone(item.phoneNumber, role, phoneType)
      : undefined,
  }));
}
