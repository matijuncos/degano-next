...
const MainInformation = ({
  ...
  const clientOptions = filteredClients
    ...
    .map((client) => ({
      value: client._id,
      label: `${client.fullName} - ${obfuscatePhone(client.phoneNumber, role, 'client')}`
    }));
  ...
  return (
    <Flex direction='column' gap='8px' mt='8px'>
      ...
      {missingFieldsCount > 0 && canEditEvents && (
        <Button
          variant='light'
          color='blue'
        ...
        selectedEvent.extraClients.map((client, index) => (
          <React.Fragment key={`extra-client-${index}`}>
            ...
            <EditableData disabled={!canEditEvents}
              ...
              value={obfuscatePhone(client.phoneNumber, role, 'client')}
            />
            {client.email && (
              <EditableData disabled={!canEditEvents}
                type='text'
                ...
                {selectedExtraClientId && (
                  <Card withBorder p='sm' bg='gray.7' color='white'>
                    ...
                    <Box pl='md'>
                      ...
                      <Text size='sm'>
                        Teléfono: {obfuscatePhone(extraClientData.phoneNumber, role, 'client')}
                      </Text>
                    </Box>
                  </Card>
                )}
...
const EventPage = () => {
  ...
  const tabContent = {
    main: (
      ...
      >
        <MainInformation selectedEvent={selectedEvent} canEditEvents={canEditEvents} role={role} />
      </PDFActions>
    ),
    bands: (
      <PDFActions
    ...
    moreInfo: (
      ...
      >
        <MoreInfoInformation selectedEvent={selectedEvent} canEditEvents={canEditEvents} />
      </PDFActions>
    ),
    equipment: (
      <PDFActions
