...
const BandList = ({ bands, onBandsChange, editing }: { bands: Band[]; onBandsChange: (bands: Band[]) => void; editing?: boolean }) => {
  ...
  return (
    <>
      { !editing &&
        <Accordion value={opened} onChange={setOpened} /*style={{ margin: '5px 0 10px' }}*/ >
        <Accordion.Item key='Banda en vivo' value='Banda en vivo'>
          ...
          <Accordion.Panel>
            <EditableBand band={selectedBand || undefined} onSave={handleSaveBand} onCancel={handleCancelBand} />
          </Accordion.Panel>
        </Accordion.Item>
      </Accordion>
      } 
      { editing && 
        <>
        {!showEditableBand && <div style={{display: 'flex', justifyContent: 'flex-end'}}>
        <IconSquareRoundedPlus onClick={() => setShowEditableBand(true)} color="green" cursor='pointer' size={30}/>

        </div>}
        {showEditableBand && <EditableBand band={selectedBand || undefined} onSave={handleSaveBand} onCancel={handleCancelBand} />}
        </>
      }
      {bands.length > 0 && (
        <div>
          ...
          <ul style={{ padding: '0px 20px 20px' }}>
            {bands.map((b, index) => (
              <li key={index} style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>
                  <strong>{b.bandName}</strong> - {b.manager} ({b.managerPhone}) {b.bandInfo ? `- ${b.bandInfo}` : ''}
                </span>
                <div>
                  <IconEdit cursor='pointer' size={22} color='#1971c2' onClick={() => handleEditBand(b)} />
                  <IconTrash cursor='pointer' size={22} color='red' onClick={() => handleDeleteBand(index)} />
                </div>
              </li>
            ))}
          </ul>
